...
in
{
  ...
  config = lib.mkIf config.yubikey.enable {
    ...
    services.udev.extraRules = ''
      ...
      # Pulling the key locks every session. DMS follows logind's Lock signal
      # through its loginctlLockIntegration setting, which the home-manager
      # activation pins so the settings dialog cannot break this rule.
